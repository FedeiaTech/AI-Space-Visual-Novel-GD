...
func _ready() -> void:
	...
	# === Lógica de atenuación al inicio ===
	...
	main_tween.tween_property(center_container, "modulate:a", 1.0, 1.0)
	main_tween.tween_callback(tween_fade_in_buttons)

func tween_fade_in_buttons():
	# Este tween maneja la animación de los botones.
	var buttons_tween = create_tween()
	
	# Con 'set_parallel(true)', todas las siguientes animaciones
	# en este tween se ejecutarán al mismo tiempo.
...
func _on_bgm_slider_changed(value: float):
	...
	# Usa remap() para mapear el valor de dB (-45 a 0) a un porcentaje (0 a 100).
