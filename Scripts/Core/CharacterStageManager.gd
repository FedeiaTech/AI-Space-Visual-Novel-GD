...
func handle_text(line: Dictionary, _is_preprocessing: bool = false) -> String:
	...
	if not speaker_key.is_empty() and speaker_key.to_upper() != "NARRATOR":
		...
		if speaker_position != "":
			...
			# --- SUB-CASO 1.2: El hablante NO ESTÁ EN ESCENA ---
			var speaker_details = Character.get_details_from_string(speaker_key)
			var speaker_enum = Character.get_enum_from_string(speaker_key)
			var has_sprite = speaker_enum != -1 and speaker_details.get("sprite_frames") != null

			if has_sprite:
				# Auto-colocar al hablante en center y re-renderizar escena
				active_characters["center"] = speaker_key
				var visuals_data = {
					"characters": active_characters.duplicate(),
					"speaker": speaker_key,
					"text": line.get("text", ""),
				...
				# Personaje sin sprite (ej. IA): habla fuera de cámara
				main_scene.set_current_speaker(null)
